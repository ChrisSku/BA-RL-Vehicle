...
class BEV(gym.Env):
    ...
    def step(self, action_in):
        ...
        reward = self.glider.position + self.battery.SOC*self.glider.position/5000
        # if len(self.velocity_traker) > 300:
        #     done = round(sum(self.velocity_traker[-30:])/30) == 0
        # if done:
        #     reward = self.glider.position
        self.state = (np.clip(self.glider.velocity, self.min_speed, self.max_speed), self.glider.acceleration, self.battery.SOC)
        self.time += 1
        return np.array(self.state), reward, done, self.glider.position
    ...
    def smooth(self, miles, smoothing_horizon=100., initial_value=0.):
    ...
    def plotTotalreward(self, totalreward):

        length = len(totalreward)
        plt.plot(totalreward)
        plt.ylabel('Reward')
        plt.xlabel('Episodes')
        plt.savefig(f'figures/{self.figuer_path}/total-{length}.png')
        plt.axes([0, length, 0, 100])
        plt.clf()
